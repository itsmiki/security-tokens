...
class SH(Token):
    def __init__(self, session: Session, name: str, description: str = None, message: str = None):
        super().__init__(session, name, tokenTypes.SH, description, message)
        self.path = None
    ...
    def create_token(self, ip, port = None) -> str:
        ...
        if not os.path.exists(os.path.dirname(os.path.abspath(__file__)) + "/sh"):
            os.mkdir(os.path.dirname(os.path.abspath(__file__)) + '/sh')
